...
int transfer_init(
  ...
  */
  ...
  /*TN added*/
  HyperInterpStruct BIS0;
  int Lsize, Lmax, x_size;
  int j;
  int *L;
  double k;
  /*double *Phi, *chireverse;*/
  /*int (*interpolate_Phi)();*/
  FILE * stream;
  ...
  */
  ...
  Lmax = ptr->L0;
  Lsize = 1;
  class_alloc(L,Lsize*sizeof(int),ptr->error_message);
  L[0] = Lmax;
  /* printf("%d %f\n",Lmax,ptr->k0); */

  class_call(hyperspherical_HIS_create(0,1.,Lsize,L,ppr->hyper_x_min,xmax,ppr->hyper_sampling_flat,Lmax+1,ppr->hyper_phi_min_abs,&BIS0,ptr->error_message),ptr->error_message,ptr->error_message);

  /*
  k = 0.01;
  x_size = ppt->tau_size;
  printf("%d\n",x_size);
  class_alloc(chireverse,x_size*sizeof(double),ptr->error_message);
  printf("A \n");
  for (j=0; j<x_size; j++) {
    chireverse[j] = k * ( tau0 - ppt->tau_sampling[x_size-1-j] );
  }
  printf("A\n");
  class_alloc(Phi,sizeof(double)*x_size,ptr->error_message);
  interpolate_Phi = hyperspherical_Hermite3_interpolation_vector_Phi;
  class_call(interpolate_Phi(&BIS0, x_size, 0, chireverse, Phi, ptr->error_message),
...
#pragma omp parallel                                                    \
  shared(tau_size_max,ptr,ppr,pba,ppt,tp_of_tt,tau_rec,sources_spline,abort,BIS,BIS0,tau0) \
...
#pragma omp flush(abort)
  ...
  /* TN */
  class_call(hyperspherical_HIS_free(&BIS0,ptr->error_message),
             ptr->error_message,
             ptr->error_message);
    ...
    if (ptr->k[index_md][index_q] <= ppt->k[index_md][ppt->k_size_cl[index_md]-1]) {
      ...
      for (index_ic = 0; index_ic < ppt->ic_size[index_md]; index_ic++) {
        ...
        for (index_tt = 0; index_tt < ptr->tt_size[index_md]; index_tt++) {
          ...
          /* TN 
          class_call(transfer_radial_coordinates(ptr,ptw,index_md,index_q,pba->conformal_age),
                     ptr->error_message,
                     ptr->error_message); */
          ...
          /* printf("%f,%f\n",pba->conformal_age,ptw->tau0_minus_tau[0]); TN */
...
int transfer_radial_coordinates(
  ...
  case 0:
    for (index_tau=0; index_tau < ptw->tau_size; index_tau++) {
      ...
      ptw->xsi[index_tau] = ptr->k0 * ptw->tau0_minus_tau[index_tau]; /* TN, k0 is the wavenumber of the distortion fields */
      ptw->keta[index_tau] = ptr->k0 * (ptr->tau0 - ptw->tau0_minus_tau[index_tau]); /* TN */
...
int transfer_radial_function(
  ...
  HyperInterpStruct * qHIS; /* TN */
  double *chi = ptw->chi;
  double *xsi = ptw->xsi; /* TN */
  double *keta = ptw->keta; /* TN */
  double *cscKgen = ptw->cscKgen;
  double *cotKgen = ptw->cotKgen;
  int j;
  double *Phi, *dPhi, *d2Phi, *chireverse;
  double *Psi, *xsireverse, *Trans; /* TN */
  FILE *stream; /* TN */
  double K=0.,k2=1.0;
  ...
  class_alloc(Phi,sizeof(double)*x_size,ptr->error_message);
  class_alloc(Psi,sizeof(double)*x_size,ptr->error_message); /* TN */
  ...
  class_alloc(xsireverse,sizeof(double)*x_size,ptr->error_message); /* TN */
  class_alloc(Trans,sizeof(double)*x_size,ptr->error_message); /* TN */
  ...
  for (j=0; j<x_size; j++) {
    ...
    xsireverse[j] = xsi[x_size-1-j]*rescale_argument; /* TN */
  ...
  */
  ...
  case SCALAR_POLARISATION_E:
    class_call(interpolate_Phi(pHIS, x_size, index_l, chireverse, Phi, ptr->error_message),
               ptr->error_message, ptr->error_message);
    /* TN start, interpolating jL(kchi) */
    class_call(interpolate_Phi(qHIS, x_size, 0, xsireverse, Psi, ptr->error_message),
               ptr->error_message, ptr->error_message);
    /* Prepare the transfer function */
    for (j=0; j<x_size; j++) {
      /*Trans[j] = 1.0;*/
      if (keta[j]<0.1) {
        Trans[j] = 1.0 - pow(keta[j],2)/10;
      } else {
        Trans[j] = 3 * ( sin(keta[j]) - cos(keta[j])*keta[j] ) / pow(keta[j],3);
      }
    }
    ...
    */
    ...
    for (j=0; j<x_size; j++)
      ...
      radial_function[x_size-1-j] = factor*cscKgen[x_size-1-j]*cscKgen[x_size-1-j]*Phi[j]*Psi[j]*Trans[x_size-1-j]*rescale_function[j];  /* TN, reverse back to the eta order */
      /* radial_function[x_size-1-j] = factor*cscKgen[x_size-1-j]*cscKgen[x_size-1-j]*Phi[j]*Psi[j]*rescale_function[j];  /* TN, reverse back to the eta order */
    /* check transfer function
    stream = fopen("file_rad.txt","w");
    for (j=0; j<x_size; j++) {
      fprintf(stream,"%f %f\n",keta[j],radial_function[j]);
    }
    fclose(stream);
    */
    break;
...
int transfer_workspace_init(
  ...
  class_alloc((*ptw)->xsi,tau_size_max*sizeof(double),ptr->error_message); /* TN */
  class_alloc((*ptw)->keta,tau_size_max*sizeof(double),ptr->error_message); /* TN */
