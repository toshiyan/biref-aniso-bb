...
int transfer_radial_coordinates(
  ...
  case 0:
    for (index_tau=0; index_tau < ptw->tau_size; index_tau++) {
      ...
      ptw->keta[index_tau] = ptr->k0 * (tau0 - ptw->tau0_minus_tau[index_tau]); /* TN */
...
int transfer_radial_function(
  ...
  class_alloc(keta,sizeof(double)*x_size,ptr->error_message); /* TN */
  ...
  */
  ...
  case SCALAR_POLARISATION_E:
    ...
    class_call(interpolate_Phi(qHIS, x_size, 0, xsireverse, Psi, ptr->error_message),
               ptr->error_message, ptr->error_message);
    for (j=0; j<x_size; j++)
      /* This transfer function is 1/3 of the true transfer function */
      Trans[j] = 3 * ( sin(keta[j])/pow(keta[j],2) - cos(keta[j])/keta[j] );
    /* stream = fopen("file.txt","w");
    for (j=0; j<x_size; j++) {
      fprintf(stream,"%f %f\n",chireverse[j],Psi[j]);
    }
